...
void n_reliable_send(n_reliable_channel_t *ch, const u8 *data, u16 len) {
    ...
    if (ch->has_unacked) return; /* stop-and-wait: wait for ack first */

    ch->reliable_sequence++;
    ch->unacked_sequence = ch->reliable_sequence;
    ch->unacked_len = len;
    if (len > 0) {
        memcpy(ch->unacked_buffer, data, len);
    }
    ch->has_unacked = true;
    ch->last_send_time = 0.0; /* will be set when actually written to packet */
}

bool n_reliable_needs_retransmit(const n_reliable_channel_t *ch, f64 now) {
    if (!ch->has_unacked) return false;
    if (ch->last_send_time <= 0.0) return true; /* never sent */
    return (now - ch->last_send_time) >= N_RELIABLE_RETRANSMIT_SEC;
}

void n_reliable_on_ack(n_reliable_channel_t *ch, u16 ack_seq) {
    if (ch->has_unacked && ack_seq == ch->unacked_sequence) {
...
void n_reliable_write_to_packet(n_reliable_channel_t *ch, n_bitwriter_t *w, f64 now) {
     ...
     */
    bool send_payload = ch->has_unacked && n_reliable_needs_retransmit(ch, now);

    u16 payload_total = 4; /* seq + ack */
    if (send_payload) {
        payload_total += ch->unacked_len;
    }

    n_msg_header_write(w, N_MSG_COMMAND, payload_total);
    if (send_payload) {
        n_write_u16(w, ch->unacked_sequence);
    } else {
        n_write_u16(w, 0); /* no data sequence */
    }
    n_write_u16(w, ch->reliable_ack);

    if (send_payload) {
        for (u16 i = 0; i < ch->unacked_len; i++) {
            n_write_u8(w, ch->unacked_buffer[i]);
        }
        ch->last_send_time = now;
    }
}
...
bool n_reliable_read_from_packet(n_reliable_channel_t *ch, n_bitreader_t *r,
    ...
    n_reliable_on_ack(ch, remote_ack);
    ...
    *out_len = 0;
    ...
    if (n_sequence_more_recent(remote_seq, ch->reliable_ack)) {
        ch->reliable_ack = remote_seq;
